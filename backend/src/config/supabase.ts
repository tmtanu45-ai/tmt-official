...
export function createSupabaseClients(): SupabaseClients {
  ...
  const required = [
    'SUPABASE_AUTH_URL', 'SUPABASE_AUTH_SERVICE_KEY',
    'SUPABASE_MATCH_URL', 'SUPABASE_MATCH_SERVICE_KEY',
    'SUPABASE_CRED_URL', 'SUPABASE_CRED_SERVICE_KEY',
    'SUPABASE_AUDIT_URL', 'SUPABASE_AUDIT_SERVICE_KEY',
    'SUPABASE_NOTIF_URL', 'SUPABASE_NOTIF_SERVICE_KEY',
  ];
  ...
  const createAdminClient = <T>(url: string, key: string) =>
    createClient<T>(url, key, {
