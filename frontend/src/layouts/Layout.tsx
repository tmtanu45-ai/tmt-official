...
export function Layout() {
  ...
  return (
    <div className="min-h-screen bg-abyss-black flex">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-abyss-charcoal border-r border-glass-border transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex flex-col h-full">
          ...
          <div className="p-6 border-b border-glass-border">
            <Link to="/dashboard" className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-neon-cyan to-neon-violet flex items-center justify-center">
                <Trophy className="w-6 h-6 text-abyss-black" />
              </div>
              <span className="font-display font-bold text-xl gradient-text">TMT</span>
            </Link>
          </div>
            ...
            {navigation.map((item) => (
              <NavLink
                ...
                className={({ isActive }) => `
                  ...
                  ${isActive ? 'text-neon-cyan bg-neon-cyan/10 border-l-2 border-neon-cyan' : ''}
                `}
                onClick={() => setSidebarOpen(false)}
              >
          ...
          <div className="p-4 border-t border-glass-border">
            <div className="flex items-center gap-3 px-3 py-2">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-neon-cyan to-neon-violet flex items-center justify-center text-abyss-black font-bold">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-ghost-gray capitalize">{user?.role?.toLowerCase()}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center justify-between h-16 px-4">
            ...
            <Link to="/dashboard" className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-neon-cyan to-neon-violet flex items-center justify-center">
                <Trophy className="w-5 h-5 text-abyss-black" />
              </div>
              <span className="font-display font-bold text-lg gradient-text">TMT</span>
            </Link>
            <div className="w-10" />
          </div>
        </header>
        ...
        <header className="hidden lg:flex items-center justify-between h-16 px-6 bg-abyss-charcoal/50 backdrop-blur-sm border-b border-glass-border">
          <div className="flex items-center gap-4">
            <h1 className="font-display text-xl font-bold gradient-text">TMT OFFICIAL</h1>
          </div>
          <div className="flex items-center gap-4">
            <NavLink
              to="/notifications"
              className="relative p-2 rounded-lg hover:bg-white/5 transition-colors text-ghost-gray hover:text-ghost-white"
            >
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-64 bg-abyss-charcoal border-r border-glass-border">
            <div className="flex flex-col h-full">
